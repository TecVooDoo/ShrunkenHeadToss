...
namespace Sisus.Init
{
	...
	#endif
	#if DEV_MODE
	[CreateAssetMenu(fileName = MENU_NAME, menuName = ValueProviderUtility.CREATE_ASSET_MENU_GROUP + MENU_NAME)]
	#endif
	public sealed class WaitForService : ScriptableObject, IValueByTypeProviderAsync, INullGuardByType
	{
		...
		/// Token that can be used to cancel the asynchronous operation (e.g. if the client gets destroyed while waiting for the service).
		/// </param>
		/// <returns>
...
#endif
			<TValue> GetForAsync<TValue>(Component client, CancellationToken cancellationToken = default) => client ? Service.GetForAsync<TValue>(client, cancellationToken: cancellationToken) : Service.GetAsync<TValue>(cancellationToken: cancellationToken);

		/// <inheritdoc/>
		bool IValueByTypeProviderAsync.IsValueTypeSupported(Type valueType) => !valueType.IsValueType && valueType != typeof(string) && valueType != typeof(object);
