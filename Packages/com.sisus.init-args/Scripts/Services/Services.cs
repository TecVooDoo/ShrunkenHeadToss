	...
	#endif
	{
		...
		NullGuardResult INullGuardByType.EvaluateNullGuard<TService>([AllowNull] Component client)
		{
			if(!AreAvailableToAnyClient())
			{
				if(!client)
				{
					return NullGuardResult.Error($"Service is only available to clients in '{ObjectNames.NicifyVariableName(toClients.ToString())}', but the client is null.");
				}
			...
			foreach(var definition in providesServices)
			{
				...
				if(!service)
				{
					return NullGuardResult.Error($"Service of type {TypeUtility.ToString(typeof(TService))} is missing.");
				}

				if(service is not TService && !ValueProviderUtility.IsValueProvider(service))
				{
					return NullGuardResult.Error($"Service {TypeUtility.ToString(service.GetType())} has defining type {TypeUtility.ToString(typeof(TService))} but is not assignable to it.");
				}
