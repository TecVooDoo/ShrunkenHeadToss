...
namespace Sisus.Init
{
	...
	public abstract class ScriptableWrapper<TWrapped> : ScriptableWrapper, IWrapper<TWrapped>, IValueProvider<TWrapped>, IValueByTypeProvider
	{
		...
		{
			if(typeof(TWrapped).IsInterface)
			{
				yield return typeof(TWrapped);
				yield break;
			}

			yield return typeof(TWrapped);

			foreach(var baseType in typeof(TWrapped).GetBaseTypes())
			{
				yield return baseType;
			}

			foreach(var interfaceType in typeof(TWrapped).GetInterfaces())
			{
		...
		/// or when a new instance of the ScriptableObject is <see cref="ScopedService{TService}.Instance">created</see>.
		/// </para>
		/// <para>
