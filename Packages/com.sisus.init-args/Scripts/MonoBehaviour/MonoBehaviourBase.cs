...
namespace Sisus.Init
{
	...
	#endif
	{
		private const float FailInitAsyncAfterSeconds = 3f;

		internal InitState initState;

		#if !UNITY_2022_2_OR_NEWER
		private protected CancellationToken destroyCancellationToken => CancellationToken.None;
		#endif
		...
		/// <summary>
		/// Should <see cref="Init(Context)"/> wait for all services to become available before proceeding with initialization?
		/// </summary>
		/// <remarks>
		/// If the object has already been initialized before the Awake event, then <see cref="Init(Context)"/> will not be executed.
		/// </remarks>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		protected virtual bool ShouldWaitForServices() => !InitAttributeUtility.TryGet(GetType(), out var attribute) || attribute.WaitForServices;
		...
		/// <summary>
		/// Executed if synchronous initialization fails during the Awake event, and <see cref="ShouldWaitForServices"/> returns <see langword="true"/>.
		/// </summary>
		...
		{
			// Default implementation just executes InitInternal every frame until it returns true.
			// This can be overridden in derived types that know the exact types of all the dependencies,
			// to actually only defer initialization until the very moment that they are all available.
			do
			{
				#if UNITY_2023_1_OR_NEWER
				...
				if(initState is not InitState.Uninitialized)
				{
					#if DEV_MODE && DEBUG_DEFERRED_ON_AWAKE
					Debug.Log($"{TypeUtility.ToString(GetType())} initState has changed to {initState}. Aborting InitAsync.", this);
					#endif
					return;
				}

				if(InitInternal(Context.Awake))
				{
					#if DEV_MODE && DEBUG_DEFERRED_ON_AWAKE
					Debug.Log($"{{TypeUtility.ToString(GetType())}} successfully initialized self asynchronously.", this);
					#endif
					return;
				}
			}
		...
		protected async void Awake()
		{
			if(initState is InitState.Uninitialized)
			{
				if(!InitInternal(Context.Awake))
				{
					...
					{
						#if DEV_MODE && DEBUG_DEFERRED_ON_AWAKE
						Debug.Log($"{TypeUtility.ToString(GetType())} could not initialize itself synchronously during Awake. Executing InitAsync because ShouldWaitForServices returned true.", this);
						#endif

						// If self-initialization failed, try a second time next frame. This helps:
						// - Give services registered using [Service(LoadScene = "...")] time to become active.
						// - Avoid execution order related issues when services are registered manually in code.
						// - Removes the need for Initializers in these cases.
						...
						// Prevent OnEnable, Start, Update etc. from being executed until all dependencies have been acquired.
						enabled = false;
						...
						{
							GuardAgainstMissingArgumentsDuringInitAsync();
						}
						#endif
						...
						#if DEV_MODE && DEBUG_DEFERRED_ON_AWAKE
						Debug.Log($"{GetType().Name} - successfully initialized self asynchronously! Executing OnAwake and setting enabled = true.", this);
						#endif
					...
					if(ShouldSelfGuardAgainstNull(this))
					{
						throw OnInitFailed(new MissingInitArgumentsException(this));
					}
					#endif
				}
			}
			...
			{
				#if DEV_MODE && DEBUG_DEFERRED_ON_AWAKE
				Debug.Log($"{GetType().Name} - won't execute OnAwake yet because initialization is still in progress. Expecting Initializer to do it manually once it is safe to do so.", this);
				#endif
				return;
			}
		...
		#if UNITY_EDITOR
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private async void GuardAgainstMissingArgumentsDuringInitAsync()
		{
			#if UNITY_2023_1_OR_NEWER
			await Awaitable.WaitForSecondsAsync(FailInitAsyncAfterSeconds);
			#else
			var stopTime = Time.realtimeSinceStartup + FailInitAsyncAfterSeconds;
			...
			if(initState is InitState.Initializing && this)
			{
				#if DEV_MODE && DEBUG_DEFERRED_ON_AWAKE
				Debug.LogError($"{GetType().Name} failed to initialize self within {FailInitAsyncAfterSeconds} seconds.", this);
				#endif
		...
		protected void HandleValidate<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument, TSeventhArgument, TEighthArgument, TNinthArgument, TTenthArgument, TEleventhArgument, TTwelfthArgument>(Context context, TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument, TFifthArgument fifthArgument, TSixthArgument sixthArgument, TSeventhArgument seventhArgument, TEighthArgument eighthArgument, TNinthArgument ninthArgument, TTenthArgument tenthArgument, TEleventhArgument eleventhArgument, TTwelfthArgument twelfthArgument)
		{
			...
			{
				throw OnInitFailed(exception);
			}
			#endif
		}
		
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private Exception OnInitFailed(Exception exception)
		{
			initState = InitState.Failed;
			#if UNITY_EDITOR
			((IInitializableEditorOnly)this).NullGuardFailedMessage = exception.ToString();
			#endif
			return exception;
		}
		...
		string IInitializableEditorOnly.NullGuardFailedMessage { get; set; } = "";
