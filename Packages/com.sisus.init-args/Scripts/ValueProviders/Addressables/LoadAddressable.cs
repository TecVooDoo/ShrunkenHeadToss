...
namespace Sisus.Init.ValueProviders
{
		...
		#endif
	{
		...
		/// <summary>
		/// True if this is an asset-based value provider, which could be shared between multiple clients.
		/// False if this is an embedded value provider with a single client.
		/// </summary>
		...
		#endif
		GetForAsync<TValue>([AllowNull] Component client, CancellationToken cancellationToken)
		{
			#if UNITY_EDITOR
		...
		/// <inheritdoc/>
		bool IValueByTypeProviderAsync.IsValueTypeSupported(Type valueType)
		{
			#if UNITY_EDITOR
			if(!Application.isPlaying)
			{
				...
				if(addressableAsset.editorAsset is SpriteAtlas spriteAtlas)
				{
					return valueType.IsInstanceOfType(spriteAtlas.GetSprite(addressableAsset.SubObjectName));
				}

				return valueType.IsInstanceOfType(addressableAsset.editorAsset);
			}
			#endif
		...
		private void OnValidate()
		{
			...
			if(addressableAsset.editorAsset is { } asset && asset)
			{
				setName = asset.name;
			}
		...
		NullGuardResult INullGuard.EvaluateNullGuard([AllowNull] Component client)
		{
			...
			{
				return NullGuardResult.Error("Invalid Addressable reference.");
			}

			if(addressableAsset.OperationHandle.IsValid() && addressableAsset.OperationHandle.OperationException is { } exception)
			{
				return NullGuardResult.Exception($"Loading addressable \"{addressableAsset.RuntimeKey}\" failed: " + exception);
			}

			if(!addressableAsset.editorAsset)
			{
				return NullGuardResult.Error($"Addressable asset \"{addressableAsset.RuntimeKey}\" not found.");
			}
