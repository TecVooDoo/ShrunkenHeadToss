...
namespace Sisus.Init.Internal
{
		...
		#endif
	{
		...
		NullGuardResult INullGuard.EvaluateNullGuard([AllowNull] Component client)
		{
			...
			if(!asset)
			{
				return NullGuardResult.Error(sceneOrAssetPath is { Length: > 0 }
					? $"Cross-scene reference asset missing: \"{sceneOrAssetPath}\" (guid: {sceneOrAssetGuid})"
					: $"Cross-scene reference asset missing. Guid: {sceneOrAssetGuid}.");
			}
			...
			foreach(var idTag in IdTag.AllTags)
			{
				if(idTag.guid == guid)
				{
					return idTag.target
						...
						: NullGuardResult.Error($"Cross-scene reference asset missing: \"{sceneOrAssetPath}\" (guid: {sceneOrAssetGuid})");
				}
			}
			...
			if(referenceType is CrossSceneReferenceType.Prefab)
			{
				...
				{
					return NullGuardResult.Error("Reference type is Prefab Instance but target is not a GameObject.");
				}
				...
				return NullGuardResult.Error($"Cross-scene reference target not found in the prefab \"{sceneOrAssetPath}\" (guid: {sceneOrAssetGuid})");
			}
			...
			if(buildIndex is -1)
			{
				return NullGuardResult.Warning($"Cross-scene reference target scene \"{sceneOrAssetPath}\" is not included in the Scene List of the active Build Profile.");
			}
			...
			for(int i = 0, count = SceneManager.sceneCount; i < count; i++)
			{
				...
				{
					return NullGuardResult.Error($"Cross-scene reference target not found in the scene \"{asset.name}\" (guid: {sceneOrAssetGuid})");
				}
			}
