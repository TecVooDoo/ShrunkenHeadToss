...
namespace Sisus.Init
{
	...
	/// <para>
	/// <see cref="Any{T}"/> did not support serializing generic types - with the exception of <see cref="List{T}"/>
	/// - in Unity 2022 and older. so this type should be used for that purpose instead, in custom initializer and such.
	/// </para>
	/// <para>
	...
	[Obsolete("Any<T> supports generic types in Unity 2023.1 and newer, so AnyGeneric<T> is no longer needed. Please use Any<T> instead.", false)]
	#endif
	[Serializable]
	#if ODIN_INSPECTOR
		...
		#endif
	{
		...
		/// <see cref="AnyGeneric{T}"/> struct with the underlying value of <paramref name="fromReference"/>.
		/// </summary>
		...
		GetValueAsync(Component client = null, Context context = Context.MainThread)
		{
			...
			return reference switch
			{
				null => value switch
				{
					null => await (client ? Service.GetForAsync<T>(client, default, context) : Service.GetAsync<T>()),
					Object unityObject when !unityObject => await (client ? Service.GetForAsync<T>(client, default, context) : Service.GetAsync<T>()),
					// Needed to support value providers in Any.Serialization.cs
				...
				_ => client ? await Service.GetForAsync<T>(client, default, context) : await Service.GetAsync<T>()
			};
		}
		...
		void ISerializationCallbackReceiver.OnBeforeSerialize()
		{
			...
			if(reference is CrossSceneReference { referenceType: CrossSceneReferenceType.None } crossSceneReference)
			{
				...
				{
					#if DEV_MODE
					Debug.Log($"CrossSceneReference {crossSceneReferenceValue.name} ({crossSceneReferenceValue.GetType().Name}) referenceType was None. Changing into a direct reference.", crossSceneReference.Value);
					#endif
					...
					{
						Debug.LogWarning($"CrossSceneReference {crossSceneReferenceValue.name} of type {crossSceneReferenceValue.GetType().Name} not convertible to {typeof(T).Name}.", crossSceneReference.Value);
					}
					#endif
				}
			}
				...
				switch(reference)
				{
					...
					case IValueProvider<T> valueProvider:
						return valueProvider.HasValueFor(client)
							...
							: NullGuardResult.Error($"{TypeUtility.ToString(valueProvider.GetType())} value of type {typeof(T).Name} is missing.");
					case IValueByTypeProvider valueProvider:
						return valueProvider.HasValueFor<T>(client)
							? NullGuardResult.Passed
							: NullGuardResult.Error($"{TypeUtility.ToString(valueProvider.GetType())} value of type {typeof(T).Name} is missing.");
