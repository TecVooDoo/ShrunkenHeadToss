...
namespace Sisus.Init
{
		...
		#endif
	{
		...
		/// <see cref="Any{T}"/> struct with the underlying value of <paramref name="fromReference"/>.
		/// </summary>
		/// <param name="fromReference"> The underlying value of the <see cref="Any{T}"/> object. </param>
		/// <returns> A new instance of the <see cref="Any{T}"/> struct. </returns>
		public static bool IsCreatableFrom([AllowNull] Object fromReference)
		{
			if(!fromReference)
			{
			...
			if(fromReference is IValueByTypeProvider valueByTypeProvider && valueByTypeProvider.IsValueTypeSupported(typeof(T)))
			{
				return true;
			}

			if(fromReference is IValueByTypeProviderAsync valueByTypeProviderAsync && valueByTypeProviderAsync.IsValueTypeSupported(typeof(T)))
			{
				return true;
			}
				...
				switch(reference)
				{
					...
					case IValueProviderAsync<T> valueProvider:
						var awaitable = valueProvider.GetForAsync(client,
							...
							client && client is MonoBehaviour monoBehaviour ? monoBehaviour.destroyCancellationToken :
							#endif
					...
					case IValueByTypeProviderAsync valueProvider:
						awaitable = valueProvider.GetForAsync<T>(client,
							#if UNITY_2022_2_OR_NEWER
							client && client is MonoBehaviour monoBehaviour2 ? monoBehaviour2.destroyCancellationToken :
							#endif
							#if UNITY_EDITOR && UNITY_2022_2_OR_NEWER
							Application.exitCancellationToken
							#else
		...
		Task<T>
		#endif
		GetValueAsync(Component client = null, Context context = Context.MainThread, CancellationToken cancellationToken = default)
		{
			// Prefer cached value over a value provider - this can help avoid issues
			// such as addressable assets being loaded more than once by the same client
			// and causing the internal counter to get incremented too many times.
			...
			return reference switch
			{
				null => value switch
				{
					null => await (client ? Service.GetForAsync<T>(client, cancellationToken, context) : Service.GetAsync<T>(cancellationToken, context)),
					Object unityObject when !unityObject => await (client ? Service.GetForAsync<T>(client, cancellationToken, context) : Service.GetAsync<T>(cancellationToken, context)),
					// Needed to support value providers in Any.Serialization.cs
				...
				IValueProviderAsync<T> valueProvider => CacheValueProviderResult(await valueProvider.GetForAsync(client, cancellationToken)),
				IValueByTypeProviderAsync valueProvider => CacheValueProviderResult(await valueProvider.GetForAsync<T>(client, cancellationToken)),
				...
				_ => client ? await Service.GetForAsync<T>(client, cancellationToken, context) : await Service.GetAsync<T>(cancellationToken, context)
			};
		}
		...
		/// <para>
		/// Local services are only available in <see cref="Context.MainThread"/> contexts.
		/// </para>
		/// </param>
		/// <returns>
		...
		public bool GetHasValue([AllowNull] Component client, Context context = Context.MainThread)
		{
			...
			{
				return nullGuard.EvaluateNullGuard(client).Type is NullGuardResultType.Passed;
			}

			if(reference is INullGuardByType nullGuardByType && reference)
			{
				return nullGuardByType.EvaluateNullGuard<T>(client).Type is NullGuardResultType.Passed;
			}
			...
			{
				// Support value providers in Any.Serialization.cs, like _String.
				IValueProvider<T> valueProvider => valueProvider.HasValueFor(client),
				_ => true,
			};
		}
		...
		void ISerializationCallbackReceiver.OnBeforeSerialize()
		{
			...
			if(reference is CrossSceneReference { referenceType: CrossSceneReferenceType.None } crossSceneReference)
			{
			...
			try
			{
				if(reference is not null && reference.GetHashCode() != 0)
				{
					switch(reference)
					{
						...
						// If value is _Null token, don't return Service, even if T is defining type of a service
						case _Null:
							return value is not null ? NullGuardResult.Passed : NullGuardResult.Error($"Value of type {typeof(T).Name} is missing.");
						// Prefer non-async value provider interfaces over async ones
						...
						case IValueProvider<T> valueProvider:
							return valueProvider.HasValueFor(client)
								? NullGuardResult.Passed : NullGuardResult.Error($"Value of type {typeof(T).Name} is missing.");
						case IValueByTypeProvider valueProvider:
							return valueProvider.HasValueFor<T>(client)
								? NullGuardResult.Passed
								: NullGuardResult.Error($"Value of type {typeof(T).Name} is missing.");
						case IValueProviderAsync<T> valueProvider:
							return valueProvider.HasValueFor(client)
								? NullGuardResult.Passed
								: NullGuardResult.Error($"Value of type {typeof(T).Name} is missing.");
				...
				if(value is null || (value is Object unityObject && !unityObject))
				{
					return (client && context.IsUnitySafeContext() ? Service.ExistsFor<T>(client) : Service.Exists<T>()) ? NullGuardResult.Passed : NullGuardResult.Error($"Value of type {typeof(T).Name} is missing.");
				}

				return value switch
				{
					// Support value providers in Any.Serialization.cs, like _String.
					IValueProvider<T> valueProvider => valueProvider.HasValueFor(client) ? NullGuardResult.Passed : NullGuardResult.Error($"Value of type {typeof(T).Name} is missing."),
