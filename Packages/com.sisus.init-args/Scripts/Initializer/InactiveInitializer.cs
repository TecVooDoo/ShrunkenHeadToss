...
namespace Sisus.Init.Internal
{
	...
	public sealed class InactiveInitializer : InactiveInitializerBaseInternal<MonoBehaviour>, IInitializable
	{
		...
		private protected override void EvaluateNullGuard(List<NullGuardResult> failures)
		{
			if(!target)
			{
				failures.Add(NullGuardResult.Error($"{nameof(InactiveInitializer)} target is missing."));
			}
			else if(target is not IInitializable)
			{
				failures.Add(NullGuardResult.Error($"{nameof(InactiveInitializer)} target {target.GetType().Name} does not implement {nameof(IInitializable)}."));
			}
		}
