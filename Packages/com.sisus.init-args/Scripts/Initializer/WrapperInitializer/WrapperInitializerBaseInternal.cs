...
namespace Sisus.Init.Internal
{
	...
	public abstract class WrapperInitializerBaseInternal<TWrapper, TWrapped> : Initializer, IInitializer<TWrapped>, IValueProvider<TWrapped>, IValueByTypeProvider
		#if UNITY_EDITOR
		...
		where TWrapper : Wrapper<TWrapped>
	{
		...
		/// <inheritdoc/>
		bool IValueByTypeProvider.IsValueTypeSupported(Type valueType) => valueType.IsAssignableFrom(typeof(TWrapped));
		...
		private protected void ThrowIfMissing<TArgument>(TArgument argument)
		{
			if(argument == Null)
			{
				throw InitializerUtility.GetMissingInitArgumentsException(GetType(), typeof(TWrapped), typeof(TArgument));
			}
		}
		#endif
		...
		void IInitializerEditorOnly.EvaluateNullGuard(List<NullGuardResult> failures) => EvaluateNullGuard(failures);
		...
		private protected abstract void EvaluateNullGuard(List<NullGuardResult> failures);
		private protected abstract void Reset();
		private protected abstract void OnValidate();
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private protected static void EvaluateNullGuard<T>(T argument, List<NullGuardResult> failures)
		{
			if(InitializerUtility.IsNull(argument)) failures.Add(NullGuardResult.Error($"Argument of type {TypeUtility.ToString(typeof(T))} is missing."));
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private protected void EvaluateNullGuard<T>(Any<T> argument, List<NullGuardResult> failures)
		{
			if(argument.EvaluateNullGuard(this) is { Type: not NullGuardResultType.Passed } failure) failures.Add(failure);
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private protected void EvaluateInitStateNullGuard(List<NullGuardResult> failures)
		{
			if(initState is InitState.Failed) failures.Add(NullGuardResult.Error(((IInitializerEditorOnly)this).NullGuardFailedMessage ?? $"Failed to initialize wrapped object of type {TypeUtility.ToString(typeof(TWrapped))}."));
		}
		#endif
	}
}