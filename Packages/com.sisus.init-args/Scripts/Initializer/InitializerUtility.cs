...
namespace Sisus.Init.Internal
{
	...
	public static class InitializerUtility
	{
		...
		internal static readonly Dictionary<Type, int> argumentCountsByIServiceInitializerTypeDefinition = new(24)
		{
		...
		internal static readonly HashSet<Type> serviceInitializerTypeDefinitions = new(12)
		{
			...
			typeof(ServiceInitializer<,,,,,>),
			typeof(ServiceInitializer<,,,,,,>),
			typeof(ServiceInitializerAsync<>),
			typeof(ServiceInitializerAsync<,>),
			typeof(ServiceInitializerAsync<,,>),
			typeof(ServiceInitializerAsync<,,,>),
			typeof(ServiceInitializerAsync<,,,,>),
		...
		public static Span<Type> GetInitializerTypes([DisallowNull] Component component)
		{
			...
			#else
			foreach(var type in TypeUtility.GetAllTypesThreadSafe(typeof(CustomInitializer<Component, object>).Assembly))
			#endif
			{
		...
		private static bool IsAsyncValueProviderOrService<TArgument>(Any<TArgument> argument)
		{
			...
			if(ServiceAttributeUtility.TryGetInfoForDefiningType(typeof(TArgument), out var serviceInfo) && serviceInfo.LoadAsync)
			{
				return true;
			}
		...
		private static void HandleNullGuardFailedMessage<TInitializer>(TInitializer initializer, GameObject gameObject, List<string> missingArgumentTypes) where TInitializer : Object, IInitializerEditorOnly
		{
			...
			{
				Debug.LogWarning($"{initializer.GetType().Name} on GameObject \"{initializer.name}\" is missing " + messageEnd, (Object)gameObject ?? initializer);
			}
		}
		...
		internal static void HandleMakeWaitForService<TArgument>(Component initializer, ref Any<TArgument> argument, WaitForService waitForService)
		{
			if(!argument.GetHasValue(initializer) ||
				(ServiceAttributeUtility.TryGetInfoForDefiningType(typeof(TArgument), out var serviceInfo)
				&& (serviceInfo.LoadAsync || serviceInfo.referenceType is ReferenceType.SceneName or ReferenceType.SceneBuildIndex)))
			{
				argument = new(waitForService);
			}
		}
		...
		internal static void Reset_Initializer<TInitializer, TClient>([DisallowNull] TInitializer initializer, [MaybeNull] GameObject gameObject) where TInitializer : Object, IInitializerEditorOnly<TClient>
		{
			...
			if(gameObject is not null)
			{
				if(TryFindTargetForInitializer(gameObject, typeof(TClient), out target, initializer.MultipleInitializersPerTargetAllowed))
				{
			...
			if(InitAttributeUtility.TryGet(initializer.GetType(), out var initAttribute) && initAttribute.nullArgumentGuard is { } initializerDefaultNullArgumentGuard)
			{
				initializer.NullArgumentGuard = initializerDefaultNullArgumentGuard;
			}
			else if(InitAttributeUtility.TryGet(target?.GetType() ?? typeof(TClient), out initAttribute) && initAttribute.nullArgumentGuard is { } clientDefaultNullArgumentGuard)
			{
				initializer.NullArgumentGuard = clientDefaultNullArgumentGuard;
			}
		}
		...
		{
			if(IsServiceDefiningType<TArgument>() || Service.ExistsFor<TArgument>(initializer))
			{
				argument = default;
				return;
			}
		...
		public static void OnInitializableReset(MonoBehaviour initializable)
		{
			...
			foreach(var initializerOnGameObject in initializersOnGameObject)
			{
				if(!initializerOnGameObject.Target && initializerOnGameObject.TargetIsAssignableOrConvertibleToType(initializableType))
				{
			...
			if(InitAttributeUtility.TryGet(initializableType, out var initAttribute))
			{
				if(initAttribute.addDefaultInitializer is false)
				{
					return;
				}

				if(initAttribute.defaultInitializerType is { } defaultInitializerType
					&& typeof(IInitializer).IsAssignableFrom(defaultInitializerType))
				{
					HandleAddInitializer(initializable, defaultInitializerType);
					return;
				}
			}
			...
			// (i.e. it's not a generic initializer like InactiveInitializer), then attach the initializer
			// to the target GameObjects alongside the Initializable by default...
			if(initializerTypes.Length is 1)
			{
				var initializerType = initializerTypes[0];
				if(!InitAttributeUtility.TryGet(initializerType, out initAttribute) || initAttribute.addDefaultInitializer is not false)
				{
					HandleAddInitializer(initializable, initializerType);
				}
			...
			{
				if(InitAttributeUtility.TryGet(initializerType, out initAttribute) && initAttribute.addDefaultInitializer is true)
				{
					HandleAddInitializer(initializable, initializerType);
					return;
				}
			...
			static void HandleAddInitializer(MonoBehaviour initializable, Type initializerType)
			{
				// Only do this if the user is (likely) doing this via the Inspector.
				// If the component was added in code, we should skip doing this, because
				// this could be done as part of an Edit Mode unit test, and there were crashing
				// issues// (in Unity 2021.3.4f1) when this was done.
				if(Array.IndexOf(Selection.gameObjects, initializable.gameObject) is -1)
				{
					return;
				}
