...
namespace Sisus.Init.Internal
{
	...
	where TClient : Object
	{
		...
		private protected TClient initTargetResult = default;
		...
		bool IValueByTypeProvider.IsValueTypeSupported(Type valueType) => valueType.IsAssignableFrom(typeof(TClient));
		...
		private protected void ThrowIfMissing<TArgument>(TArgument argument)
		{
			...
			{
				throw InitializerUtility.GetMissingInitArgumentsException(this, typeof(TArgument));
			}
			#endif
		}
		...
		{
			if(initState is InitState.Failed) failures.Add(NullGuardResult.Error(((IInitializerEditorOnly)this).NullGuardFailedMessage ?? $"{GetType().Name} failed to initialize client of type {TypeUtility.ToString(typeof(TClient))}."));
		}
		#endif
	}
}