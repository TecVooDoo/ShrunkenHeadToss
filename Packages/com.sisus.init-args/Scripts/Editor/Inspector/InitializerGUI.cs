	...
	public sealed class InitializerGUI : IDisposable
	{
		...
		public void OnInspectorGUI()
		{
			...
			try
			{
				...
				{
					nullGuardFailures.Add(NullGuardResult.ClientHidesAwake);
				}
				...
				{
					var hidesAwakeFailureIndex = nullGuardFailures.FindIndex(x => x == NullGuardResult.ClientHidesAwake);
					if(hidesAwakeFailureIndex is not -1)
					{
						DrawHelpBox(MessageType.Warning, new(nullGuardFailures[hidesAwakeFailureIndex].Message), TargetHasMissingDependenciesHelpURL);
					}

					if(hidesAwakeFailureIndex is -1 || nullGuardFailures.Count > 1)
					{
