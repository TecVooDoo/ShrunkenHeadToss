...
namespace Sisus.Init.EditorOnly
{
	...
	{
		private const float VisualElementHorizontalPadding = 47f;

		public static VisualElement InitializerEditorElement { get; private set; }
		...
		private VisualElement initializerEditorRootVisualElement;
		...
		protected virtual void Setup()
		{
			...
			initializerGUI = new(SerializedObject, targets.Select(GetInitializable).ToArray(), initParameterTypes);
			...
			parent.RegisterCallback<GeometryChangedEvent>(UpdateElementDimensions);
			MarkDirtyLayout();

			void SetupElement(GeometryChangedEvent @event)
			{
				...
				if(initializerEditorRootVisualElement is null)
				{
					var initializerEditor = initializerGUI.GetOrCreateInitializerEditor();
					if(!initializerEditor)
					{
						return;
					}

					initializerEditorRootVisualElement = initializerEditor.CreateInspectorGUI();
					initializerEditor.Repaint();
				}

				if(initializerEditorRootVisualElement is not null)
				{
					Add(initializerEditorRootVisualElement);
					initializerEditorRootVisualElement.visible = false;
					initializerEditorRootVisualElement.style.width = parent.resolvedStyle.width - VisualElementHorizontalPadding;
				}
			}
		...
		private void PreOnGUISetup(Type baseType)
		{
			...
			{
				// If an Initializer is removed or added, we need to update the value of drawInitSection and repaint the GUI.
				ObjectChangeEvents.changesPublished -= OnAnyObjectChanged;
				ObjectChangeEvents.changesPublished += OnAnyObjectChanged;
			}
		}

		private void OnAnyObjectChanged(ref ObjectChangeEventStream stream)
		{
			for(int s = stream.length - 1; s >= 0; s--)
			{
				var type = stream.GetEventType(s);
				if(type is ObjectChangeKind.ChangeGameObjectStructure)
				{
					stream.GetChangeGameObjectStructureEvent(s, out ChangeGameObjectStructureEventArgs data);
					if(EditorUtility.
						#if UNITY_6000_3_OR_NEWER
						EntityIdToObject
						#else
					...
					{
						if(target is Component component && component && ReferenceEquals(component.gameObject, changedGameObject))
						{
							UpdateDrawInitSection();
							return;
						}
					}
				}
				...
				{
					stream.GetChangeGameObjectOrComponentPropertiesEvent(s, out ChangeGameObjectOrComponentPropertiesEventArgs data);
					if(EditorUtility.
						#if UNITY_6000_3_OR_NEWER
						EntityIdToObject
						#else
						...
						(data.instanceId)
						#endif
						is not Component changedComponent)
					{
						continue;
					}

					if(changedComponent is IInitializer)
					{
						UpdateDrawInitSection();
						return;
					}
					...
					foreach(var target in targets)
					{
						if(ReferenceEquals(changedComponent, target))
						{
							UpdateDrawInitSection();
							return;
						}
			...
			void UpdateDrawInitSection()
			{
				var setDrawInitSection = ShouldDrawInitSection();
				if(drawInitSection != setDrawInitSection)
				{
					drawInitSection = setDrawInitSection;
					Repaint();
				}
			}
		}
		...
		public override void OnBeforeInspectorGUI()
		{
			...
			{
				#if DEV_MODE
				Debug.Log("OnBeforeInspectorGUI called with target null. Exiting early.");
				#endif
				return;
			}
		...
		protected virtual void DrawInitializerGUI()
		{
			...
			if(drawInitSection)
			{
				if(initializerEditorRootVisualElement is not null)
				{
					initializerEditorRootVisualElement.visible = true;
					InitializerEditorElement = initializerEditorRootVisualElement;
				}
				...
				{
					InitializerEditorElement = null;
				}
			}
			else if(initializerEditorRootVisualElement is not null)
			{
				initializerEditorRootVisualElement.visible = false;
			}
		...
		public override void OnAfterInspectorGUI()
		{
			...
			{
				#if DEV_MODE
				Debug.Log("OnAfterInspectorGUI called with target null. Exiting early.");
				#endif
				return;
			}
		...
		protected override void Dispose(bool disposeManaged)
		{
			...
			scheduledItem?.Pause();
			DisposeInitializerGUI();
			parent?.UnregisterCallback<GeometryChangedEvent>(UpdateElementDimensions);
			ObjectChangeEvents.changesPublished -= OnAnyObjectChanged;
		...
		private void UpdateElementDimensions(GeometryChangedEvent @event)
		{
			if(initializerGUI is null)
			{
				parent?.UnregisterCallback<GeometryChangedEvent>(UpdateElementDimensions);
		...
		private void DisposeInitializerGUI()
		{
			...
			Debug.Log($"DisposeInitializerGUI({TypeUtility.ToString(initializerGUI.Initializers.FirstOrDefault()?.GetType())})");
			#endif
