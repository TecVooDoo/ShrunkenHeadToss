...
using CustomEditorUtility = Sisus.Shared.EditorOnly.CustomEditorUtility;
...
namespace Sisus.Init.EditorOnly.Internal
{
	...
	internal sealed class AnyPropertyDrawer : PropertyDrawer
	{
		...
		private static readonly Dictionary<Type, bool> containsSerializableFieldsCache = new();
		public static NullGuardResult CurrentlyDrawnItemNullGuardResult { get; set; }
		...
		public override void OnGUI(Rect position, SerializedProperty anyProperty, GUIContent label)
		{
			...
			{
				#if DEV_MODE && DEBUG_ENABLED
				if(!state.IsValid()) { Debug.LogWarning($"{label?.text ?? anyProperty.propertyPath} State.IsValid:false with Event.current:{Event.current?.type.ToString() ?? "None"}"); }
				else if(state.anyProperty.serializedObject != anyProperty.serializedObject) { Debug.Log($"{label?.text ?? anyProperty.propertyPath} State.serializedObject has changed. Event.current:{Event.current?.type.ToString() ?? "None"}"); }
				else { Debug.Log($"{label?.text ?? anyProperty.propertyPath} State.ValueHasChanged:true with Event.current:{Event.current?.type.ToString() ?? "None"}"); }
				#endif
			...
			state.height = DrawValueField(position, anyProperty, state, label);
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			...
			if(!states.TryGetValue(property.propertyPath, out State state))
			{
				state = new(property, fieldInfo);
				states.Add(property.propertyPath, state);
			}
		...
		public static Type GetAnyTypeFromField(FieldInfo fieldInfo)
		{
			...
			if(typeof(ICollection).IsAssignableFrom(fieldType) && fieldType.IsGenericType)
			{
		...
		private float DrawValueField(Rect position, SerializedProperty anyProperty, State state, GUIContent label)
		{
			...
			if(drawAsObjectField)
			{
				...
				if(referenceValue && controlRect.width > 35f)
				{
					if(referenceValue is INullGuard nullGuard
						&& nullGuard.EvaluateNullGuard(firstTargetObject as Component) is { Type: not NullGuardResultType.Passed } failure)
					{
						DrawNullGuardFailure(ref controlRect, failure);
					}
					else if(referenceValue is INullGuardByType nullGuardByType
						&& nullGuardByType.EvaluateNullGuard(state.valueType ?? typeof(void), firstTargetObject as Component) is { Type: not NullGuardResultType.Passed } byTypeFailure)
					{
						DrawNullGuardFailure(ref controlRect, byTypeFailure);
					}

					static void DrawNullGuardFailure(ref Rect controlRect, NullGuardResult failure)
					{
						var icon = failure.Type is NullGuardResultType.Error ? Styles.ErrorIcon.image : Styles.WarningIcon.image;
						var content = new GUIContent(icon, failure.Message);
						var iconRect = controlRect;
						iconRect.width = EditorGUIUtility.singleLineHeight;
						GUI.Label(iconRect, content);
						controlRect.x += EditorGUIUtility.singleLineHeight + 2f;
						controlRect.width -= EditorGUIUtility.singleLineHeight + 2f;
					}
				}

				var drawTypeDropdown = state.drawTypeDropdownButton && objectReferenceValueIsNull && (!draggingAssignableObject || !managedValueIsNull);
				if(drawTypeDropdown)
				{
				...
				if(referenceValue)
				{
					...
					{
						if(crossSceneReference.referenceType is CrossSceneReferenceType.None)
						{
							#if DEV_MODE
							Debug.Log($"CrossSceneReference {crossSceneReference.Value?.name} ({crossSceneReference.Value?.GetType().Name}) referenceType was None. Changing into a direct reference.", GetGameObject(firstTargetObject));
							#endif
					...
					else if(CrossSceneReference.Detect(firstTargetObject, referenceValue))
					{
						...
						CrossSceneReferenceGUI.Assign(referenceProperty, referenceValue);
						GUIUtility.ExitGUI();
					}
				}
				...
				EditorGUI.indentLevel = 0;
				if(state.draggedObjectIsAssignable is true && CrossSceneReference.Detect(firstTargetObject, DragAndDrop.objectReferences[0]))
				{
					if(TryGetNullArgumentGuardBasedControlTint(out Color setGUIColor))
					{
						GUI.color = setGUIColor;
					...
					if(setReferenceValue != referenceValue && setReferenceValue && TryGetAssignableType(state, setReferenceValue, out _))
					{
						...
						#if DEV_MODE && DEBUG_CROSS_SCENE_REFERENCES
						Debug.Log($"Cross scene reference assigned.\nClient {firstTargetObject.name} ({firstTargetObject.GetType().Name}) scene: {GetScene(firstTargetObject)}\nReference:{referenceValue.name} ({referenceValue.GetType().Name}) scene: {GetScene(referenceValue)}", GetGameObject(firstTargetObject));
						#endif
				...
				else if(state.draggedObjectIsAssignable is true && CrossSceneReferenceGUI.IsPartOfDifferentPrefabAsset(firstTargetObject, DragAndDrop.objectReferences[0]))
				{
					if(TryGetNullArgumentGuardBasedControlTint(out Color setGUIColor))
					{
						GUI.color = setGUIColor;
					...
					if(setReferenceValue != referenceValue && setReferenceValue && TryGetAssignableType(state, setReferenceValue, out _))
					{
						referenceValue = setReferenceValue;
						#if DEV_MODE && DEBUG_CROSS_SCENE_REFERENCES
						Debug.Log($"Prefab reference assigned.\nClient {firstTargetObject.name} ({firstTargetObject.GetType().Name}) scene: {GetScene(firstTargetObject)}\nReference:{referenceValue.name} ({referenceValue.GetType().Name}) prefab: {AssetDatabase.GetAssetPath(referenceValue)}");
						#endif

						CrossSceneReferenceGUI.ShowPickPrefabReferenceTypeDropdown(referenceProperty, referenceValue, controlRect);
					}
			   ...
			   && valueProperty.propertyType != SerializedPropertyType.String)
			{
				...
				const float ARRAY_SIZE_CONTROL_WIDTH = 55f;
			...
			var isSerializableByUnity = TypeUtility.IsSerializableByUnity(assignedInstanceType);
			if(!isSerializableByUnity)
			{
				if(ContainsSerializableFields(assignedInstanceType))
				{
		...
		/// <summary>
		/// Checks if the given type contains any fields that are public or marked with [SerializeField] or [SerializeReference].
		/// </summary>
		/// <remarks>
		/// Does not check if the type contains [Serializable] nor derives from UnityEngine.Object.
		/// </remarks>
		/// <param name="assignedInstanceType"></param>
		private bool ContainsSerializableFields(Type assignedInstanceType)
		{
			if(containsSerializableFieldsCache.TryGetValue(assignedInstanceType, out var contains))
			{
				return contains;
			}
			
			foreach(var fieldInfo in assignedInstanceType.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
			{
				if(fieldInfo.IsPublic || fieldInfo.IsDefined(typeof(SerializeField), true) || fieldInfo.IsDefined(typeof(SerializeReference), true))
				{
					containsSerializableFieldsCache.Add(assignedInstanceType, true);
					return true;
				}
			}

			containsSerializableFieldsCache.Add(assignedInstanceType, false);
			return false;
		}
		...
		void DrawObjectField(Rect controlRect, Object referenceValue, State state)
		{
			...
			if(!state.valueType.IsInstanceOfType(newReferenceValue))
			{
				...
				{
					var isCreatableFromMethod = state.anyType.GetMethod(nameof(Any<object>.IsCreatableFrom), BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
					bool isCreatableFrom = (bool)isCreatableFromMethod.Invoke(null, new object[] { newReferenceValue });
					if(!isCreatableFrom)
					{
						state.referenceProperty.objectReferenceValue = referenceValue;
						return;
		...
		public static void Dispose(SerializedProperty anyProperty)
		{
			...
			if(target is not null && statesByTarget.TryGetValue(target, out var states) &&
				states.Remove(anyProperty.propertyPath, out var state))
			{
				state.Dispose();
			}
		}
		...
		private sealed class State
		{
			...
			public State(SerializedProperty anyProperty, FieldInfo fieldInfo)
			{
				...
				if(InitParameterGUI.NowDrawing is not null)
				{
					label = InitParameterGUI.NowDrawing.label;
				}
				else
				{
			...
			public void Update(bool forceDeepUpdate = false)
			{
				...
				if(referenceProperty.objectReferenceValue is _Null nullObject && nullObject && !valuePropertyIsNull && valueProperty.propertyType is SerializedPropertyType.ManagedReference)
				{
				...
				drawNullOption = !valueType.IsValueType && valueType != typeof(string);
			...
			private void RebuildDefiningTypeAndDiscardButtons(out bool drawTypeDropdownButton, out bool drawDiscardButton)
			{
				...
				var dataSource = new AnyFieldTypeOptionsDataSource("Select Value", valueProperty, valueType, isService, canBeUnityObject, drawNullOption, new() { instanceType });
				if(!dataSource.HasMultipleOptions)
				{
				...
				{
					typeDropdownButton = new(GUIContent.none, new(buttonText), dataSource, OnSelectedItemChanged);
				...
				if((managedValueIsNull && drawObjectField) || !dataSource.HasMultipleNonValueProviderOptions)
				{
					typeDropdownButton = new(GUIContent.none, GUIContent.none, dataSource, OnSelectedItemChanged);
					discardValueButton = null;
					drawTypeDropdownButton = true;
					drawDiscardButton = false;
					return;
				...
				if(managedValueIsNull)
				{
					typeDropdownButton = new(prefixLabel, new(buttonText), dataSource, OnSelectedItemChanged);
					discardValueButton = null;
					drawTypeDropdownButton = true;
					drawDiscardButton = false;
					return;
				...
				typeDropdownButton = null;
				discardValueButton = new($"Click to discard value of type {TypeUtility.ToString(managedValue.GetType())}.", () => OnSelectedItemChanged(null));
			...
			{
				#if DEV_MODE && DEBUG_SET_VALUE_TYPE
				Debug.Log($"Setting user selected value type to {TypeUtility.ToString(setType)} for {anyProperty.propertyPath} with propertyType:{valueProperty?.propertyType.ToString() ?? "n/a"}");
				#endif
				...
				if(setType is null)
				{
					if(!valuePropertyIsNull && valueProperty.propertyType is SerializedPropertyType.ManagedReference)
					{
				...
				else if(typeof(Object).IsAssignableFrom(setType))
				{
					...
					if(setType == typeof(Object))
					{
						if(isService)
						{
							...
							#if DEV_MODE
							Debug.Log($"Assigning _Null to {referenceProperty.propertyPath} because user selected type {TypeUtility.ToString(setType)} for the field. This should cause the field to be drawn as an Object reference field.", referenceProperty.serializedObject.targetObject);
							#endif
							...
							&& typeof(ScriptableObject).IsAssignableFrom(setType))
					{
						...
						bool useSingleSharedInstance = ValueProviderUtility.TryGetSingleSharedInstanceSlow(setType, out var singleSharedInstance);
				...
				else if(!valuePropertyIsNull && valueProperty.propertyType is not SerializedPropertyType.ManagedReference)
				{
				...
				else
				{
					if(!valuePropertyIsNull)
					{
						valueProperty.SetValue(InitializerEditorUtility.CreateInstance(setType));
					}
			...
			private void UpdateValueProviderEditor()
			{
				...
				if(referenceProperty.objectReferenceValue is ScriptableObject valueProvider
				   && ShouldDrawEmbedded(valueProvider, out var valueProviderEditor))
				{
				...
				static bool ShouldDrawEmbedded(ScriptableObject valueProvider, out Editor valueProviderEditor)
				{
					if(!valueProvider
					|| !ValueProviderUtility.IsValueProvider(valueProvider)
					|| valueProvider is _Null)
					{
						valueProviderEditor = null;
						return false;
					}

					valueProviderEditor = Editor.CreateEditor(valueProvider, null);
					if(!valueProviderEditor)
					{
						return false;
					}

					if(!CustomEditorUtility.IsDefaultOrOdinEditor(valueProviderEditor.GetType()))
					{
						return true;
					}

					if(valueProvider.GetType().IsDefined(typeof(ValueProviderMenuAttribute)))
					{
						if(!AssetDatabase.Contains(valueProvider) // Draw embedded value providers inlined
					...
					Object.DestroyImmediate(valueProviderEditor);
					return false;
				}
			}
			...
			private void OpenCustomContextMenu(Rect rect)
			{
				...
				else if(isService)
				{
					...
					{
						#if DEV_MODE
						Debug.Log($"Assigning _Null to {referenceProperty.propertyPath} because user selected 'Set Null' from the context menu. This should cause the field to be drawn as an Object reference field.", referenceProperty.serializedObject.targetObject);
						#endif

						referenceProperty.objectReferenceValue = ScriptableObject.CreateInstance<_Null>();
					});
				}
