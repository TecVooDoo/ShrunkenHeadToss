	...
	internal static class ScriptGenerator
	{
		...
		private static string CreateInitializer(string initializerPath, Type clientType = null)
		{
			...
			if(clientType != null)
			{
				...
				if(!TryGetInitParameters(clientType, parametersTypes, out initParameters))
				{
					initParameters = new ParameterInfo[parametersTypes.Length];
				}
			}
			else
			{
				...
				initParameters = Array.Empty<ParameterInfo>();
			}

			string genericArguments;
			string members = "";
			int parameterCount = parametersTypes.Length;
			var namespaces = new HashSet<string>() { "Sisus.Init" };
			...
			if(parameterCount > 0)
			{
				for(int i = 0; i < parameterCount; i++)
				{
					...
					var parameter = initParameters[i];
					var parameterName = parameter?.Name;
					var parameterTypeName = NameOf(parameterType, namespaces);
					parameterTypeNames[i] = parameterTypeName;

					string name;
					if(TryGetArgumentTargetMember(clientType, parameterType, parameter, i, out var member))
					{
						...
						name = parameterName ?? GetLabel(member.Name).Replace(" ", "");
					}
					else if(parameterName is not null)
					{
						name = parameterName;
					}
					else
				...
				genericArguments = string.Join(", ", parameterTypeNames);
			}
			else
			{
			...
			string template = InitializerTemplate;

			switch(initializerType)
			{
				case InitializerType.WrapperInitializer:
					baseClassName = "WrapperInitializer";
					...
					template = WrapperInitializerTemplate;
					...
					string parameterList = string.Join(", ", parameterTypeNames.Select((typeName, index) => typeName + " " + parameterNamesLowerCase[index]));
				...
				case InitializerType.VisualElementInitializer:
					...
					foreach(string fieldName in initializerFieldNames)
					{
						...
						initTargetParameters += $", {parameterTypeNames[index]} {variableName}";
						index++;
					}

					baseClassName = "VisualElementInitializer";
		...
		private static string NameOf([NotNull] Type type, HashSet<string> namespaces)
		{
