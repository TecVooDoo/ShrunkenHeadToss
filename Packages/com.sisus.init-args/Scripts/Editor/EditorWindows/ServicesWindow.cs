...
namespace Sisus.Init.EditorOnly.Internal
{
	internal sealed class ServicesWindow : EditorWindow
	{
		...
		private bool autoRebuildEnabled;
		...
		private void DrawTabBar()
		{
			...
			var setShowInactive = GUILayout.Toggle(showInactive, showInactive ? inactiveLabel : activeLabel, EditorStyles.toolbarButton, GUILayout.Width(55f));
		...
		private void DrawServicesList(List<Service.ActiveServiceInfo> activeServices, IEnumerable<Service.ActiveServiceInfo> inactiveServices)
		{
			using var inactiveServicesEnumerator = inactiveServices.GetEnumerator();
			var hasInactiveServices = inactiveServicesEnumerator.MoveNext();

			if(activeServices.Count is 0 && !hasInactiveServices)
			{
				GUILayout.Label("No services to list.", EditorStyles.largeLabel);
			}

			GUIContent label = new();
			foreach(var serviceInfo in activeServices)
			{
				DrawService(serviceInfo);
			}

			if(hasInactiveServices && showInactive)
			{
				do
				{
					DrawService(inactiveServicesEnumerator.Current);
				}
				while(inactiveServicesEnumerator.MoveNext());
			}

			void DrawService(Service.ActiveServiceInfo serviceInfo)
			{
				if(serviceInfo.IsAsset)
				{
					return;
				}

				var service = serviceInfo.ServiceOrProvider;
				var isNull = service == Null;
				if(isNull && serviceInfo.InitFailReason is ServiceInitFailReason.None)
				{
					return;
				}

				if(filter.Length > 0 && serviceInfo.Label.text.IndexOf(filter, StringComparison.OrdinalIgnoreCase) is -1 && serviceInfo.ClientsText.IndexOf(filter, StringComparison.OrdinalIgnoreCase) == -1)
				{
					return;
				}

				Object unityObject = null;
				var isUnityObject = !isNull && TryGetUnityObject(service, false, out unityObject);
				var registerer = serviceInfo.Registerer;
				var registererIsInactive = registerer && registerer is Behaviour { isActiveAndEnabled : false };
				var isActive = !isNull && !registererIsInactive && (!isUnityObject || (unityObject && unityObject is not Behaviour { isActiveAndEnabled : false }));
				...
				{
					if(serviceInfo.Registerer is Services services)
					{
						serviceInfo.Setup(services.providesServices.Where(x => ReferenceEquals(x.service, service)).Select(x => x.DefiningType).Where(x => x is not null).ToArray());
					}
					else if(serviceInfo.Registerer is ServiceTag serviceTag)
					{
						serviceInfo.Setup(new[] { serviceTag.DefiningType });
					}
					else
					{
				...
				{
					label.image = EditorGUIUtility.ObjectContent(null, typeof(MonoScript)).image;
				}
				
				GUI.color = isActive ? Color.white : new(1f, 1f, 1f, 0.5f);
				if(GUILayout.Button(label, Styles.RichTextLabel, expandWidth))
				{
					switch(Event.current.button)
					{
						...
						case 1:
							...
							if(serviceInfo.Registerer && !ReferenceEquals(service, serviceInfo.Registerer))
							{
								menu.AddItem(new("Ping Registerer"), false, () => PingObject(serviceInfo.Registerer));
							}
							else if(ServiceAttributeUtility.TryGetInfoForDefiningType(serviceInfo.definingType ?? serviceInfo.ConcreteType, out var globalServiceInfo))
							{
								if(globalServiceInfo.classWithAttribute is { } classWithAttribute && classWithAttribute != serviceInfo.ConcreteType)
								{
									if(Find.Script(classWithAttribute, out var scriptWithAttribute))
									{
										menu.AddItem(new("Ping Type With [Service] Attribute"), false, () => PingObject(scriptWithAttribute));
									}
								}
							}

							if(serviceInfo.ConcreteType is { } concreteType && Find.Script(concreteType, out var script))
							{
								menu.AddItem(new("Ping Script"), false, () => PingObject(script));
							}
				...
				if(serviceInfo.InitFailReason is not ServiceInitFailReason.None)
				{
					label.text = serviceInfo.InitFailReason.ToString();
					label.tooltip = "Service initialization failed!\n\n" + serviceInfo.Label.tooltip;
					label.image = Styles.ErrorIcon.image;
					GUI.color = Color.red;

					if(GUILayout.Button(label, EditorStyles.label))
					{
						GUIUtility.systemCopyBuffer = serviceInfo.Label.tooltip;
						Debug.Log(serviceInfo.Label.tooltip);
					}
		...
		private void DrawClientsList()
		{
			...
			foreach(var clientInfo in clients)
			{
				...
				var unityObject = clientInfo.unityObject;
				...
				var isActive = !isUnityObject || (unityObject && (unityObject is not Behaviour behaviour || behaviour.isActiveAndEnabled));
				if(!isActive && !showInactive)
				{
					continue;
				}
		...
		{
			if(TryGetUnityObject(service, true, out var unityObject))
			{
				EditorGUIUtility.PingObject(unityObject is Component component ? component.gameObject : unityObject);
			}
		}
		...
		private struct ClientInfo : IEquatable<ClientInfo>
		{
			...
			public readonly string InitState => client.InitState switch
			{
				Init.Internal.InitState.Uninitialized => "Uninitialized",
				Init.Internal.InitState.Initializing => "Initializing",
				Init.Internal.InitState.Initialized => "Initialized",
				Init.Internal.InitState.Failed => "Failed",
				_ => client.InitState.ToString()
			};

			public readonly Color InitStateColor => client.InitState switch
			{
				Init.Internal.InitState.Uninitialized => Color.grey,
				Init.Internal.InitState.Initializing => Color.grey,
				Init.Internal.InitState.Initialized => Color.white,
				Init.Internal.InitState.Failed => Color.red,
				_ => Color.white
			};
			...
			public ClientInfo(IInitializableEditorOnly client) : this()
			{
				...
				label = new(()=> new(TypeUtility.ToString(client.GetType()), (obj ? EditorGUIUtility.ObjectContent(obj, obj.GetType()) : EditorGUIUtility.ObjectContent(null, typeof(MonoScript))).image));
			}
